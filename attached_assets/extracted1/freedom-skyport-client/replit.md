...
- **Dev-Only Feature**: Added role switcher UI component (bottom-right corner) visible only in development mode
  - Easily switch between Owner, Admin, and Instructor roles without database changes
  - Uses localStorage to override role display (`dev_role_override` key)
  - Automatically refreshes page when role changes
  - Styled with yellow warning colors to indicate dev-only feature
- **Updated useUserRole hook**: 
  - Checks for dev override in non-production environments
  - Falls back to actual database roles in production
  - Added instructor role support
- **Updated Dashboard routing**: Now supports all three roles (owner, admin, instructor)
  - Instructor dashboard shows placeholder for future implementation

### Owner Dashboard & Access Controls Implementation
- **Simplified Owner Dashboard**: Single "Your Aircraft" card with embedded Quick Actions
  - Reduced clutter: main dashboard shows only aircraft summary and two primary action buttons
  - Moved additional sections (billing, service timeline, docs, credits) to new `/more` page
  - Maintained existing action handlers for "Prepare Aircraft for Flight" and "Request Service"
- **Server-Side Access Control (ACL)**:
  - Added `getSessionUser()` helper in server/replitAuth.ts to extract user ID and role
  - Enforced owner scoping across all API routes:
    - Aircraft: Owners can only view/modify their own aircraft; admins can view all
    - Service Requests: Owners can only create/view/modify their own requests
    - Service Tasks: Scoped to owner's aircraft
    - Memberships & Invoices: Scoped to owner's ID
  - Consistent pattern: owners see their data, admins see all data
- **Testing Infrastructure**:
  - Added Vitest for unit tests with happy-dom environment
  - Added Playwright for e2e testing
  - Created `npm run test:unit` and `npm run test:e2e` scripts
  - Unit tests for action payloads (prepare aircraft, request service)
  - E2e tests for owner scoping validation (placeholders)
...
- ✅ Owner Dashboard simplified with access controls enforced
