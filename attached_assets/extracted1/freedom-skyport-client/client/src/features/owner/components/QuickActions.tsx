...
export function QuickActions({ aircraftId, userId, aircraftData }: QuickActionsProps) {
  ...
  const [prepForm, setPrepForm] = useState({
    ...
    airport: "",
    requested_departure: "",
    fuel_option: "tabs", // tabs, full, or custom
    fuel_quantity: "",
    o2_topoff: false,
    tks_topoff: false,
    gpu_required: false,
  ...
  const handlePrepareAircraft = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = {
        ...
        fuel_grade: null,
        fuel_quantity: prepForm.fuel_option === "custom" ? prepForm.fuel_quantity : prepForm.fuel_option,
      ...
      await queryClient.invalidateQueries({ queryKey: ["/api/service-tasks"] });
    } catch (error) {
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-3">
                  <div className="flex gap-2">
                    <Button
                      type="button"
                      variant={prepForm.fuel_option === "tabs" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setPrepForm({ ...prepForm, fuel_option: "tabs", fuel_quantity: "" })}
                      data-testid="button-fuel-tabs"
                      className="flex-1"
                    >
                      Fill to Tabs
                    </Button>
                    <Button
                      type="button"
                      variant={prepForm.fuel_option === "full" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setPrepForm({ ...prepForm, fuel_option: "full", fuel_quantity: "" })}
                      data-testid="button-fuel-full"
                      className="flex-1"
                    >
                      Full Tanks
                    </Button>
                    <Button
                      type="button"
                      variant={prepForm.fuel_option === "custom" ? "default" : "outline"}
                      size="sm"
                      onClick={() => setPrepForm({ ...prepForm, fuel_option: "custom" })}
                      data-testid="button-fuel-custom"
                      className="flex-1"
                    >
                      Custom
                    </Button>
                  </div>
                  {prepForm.fuel_option === "custom" && (
                    <Input
