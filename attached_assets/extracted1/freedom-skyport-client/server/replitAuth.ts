...
export async function getSessionUser(req: any): Promise<{ id: string; role: 'owner' | 'admin' } | null> {
  ...
  const role = roles.includes('admin') ? 'admin' : roles.includes('owner') ? 'owner' : null;
